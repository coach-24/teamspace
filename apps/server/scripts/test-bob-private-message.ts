...
const main = async () => {
  ...
  console.log("\n🚀 Bob is trying to send a message to Secret Project...");
  ...
  const response = await fetch(
    `${API_URL}/api/channels/${channelId}/messages`,
    {
      ...
      body: JSON.stringify({
        content: "Bob should NOT be able to send this.",
      }),
    },
  );
  ...
  if (
    ...
    console.log("\n🎉 PRIVATE MESSAGE RBAC PASSED!");
    console.log("Bob cannot send messages to the private channel.");
  } else {
    console.log("\n❌ PRIVATE MESSAGE RBAC FAILED.");
    process.exit(1);
  }
};
